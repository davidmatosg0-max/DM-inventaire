...
export function GestionUnidades() {
  ...
  const handleGuardar = () => {
    if (!formData.nombre.trim()) {
      toast.error(t('inventory.unitManagement.errors.nameRequired'));
      return;
    }

    if (!formData.abreviatura.trim()) {
      toast.error(t('inventory.unitManagement.errors.abbreviationRequired'));
      return;
    }
    ...
    if (guardado) {
      toast.success(modoEdicion ? t('inventory.unitManagement.toasts.updated') : t('inventory.unitManagement.toasts.created'));
      setDialogOpen(false);
      cargarUnidades();
    } else {
      toast.error(t('inventory.unitManagement.errors.saveFailed'));
    }
  };

  const handleEliminar = (unidad: Unidad) => {
    if (confirm(t('inventory.unitManagement.confirmDelete', { name: unidad.nombre }))) {
      const eliminado = eliminarUnidad(unidad.id);
      
      if (eliminado) {
        toast.success(t('inventory.unitManagement.toasts.deleted'));
        cargarUnidades();
      } else {
        toast.error(t('inventory.unitManagement.errors.deleteFailed'));
      }
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg mb-1" style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 600 }}>
            {t('inventory.unitManagement.title')}
          </h3>
      ...
      {unidades.length === 0 && (
        <div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
          ...
          <p className="text-[#666666] mb-2">{t('inventory.unitManagement.empty')}</p>
          <Button
            onClick={handleAbrirNuevo}
            variant="outline"
            className="mt-2"
          >
            <Plus className="w-4 h-4 mr-2" />
            {t('inventory.unitManagement.createFirst')}
          </Button>
        </div>
      )}
        ...
        <DialogContent className="max-w-md" aria-describedby="gestion-unidad-description">
          <DialogHeader>
            <DialogTitle style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 600 }}>
              {modoEdicion ? t('inventory.unitManagement.dialog.editTitle') : t('inventory.unitManagement.dialog.createTitle')}
            </DialogTitle>
            <DialogDescription id="gestion-unidad-description">
              {modoEdicion ? t('inventory.unitManagement.dialog.editDescription') : t('inventory.unitManagement.dialog.createDescription')}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label>{t('inventory.unitManagement.form.name')}</Label>
              <Input
                value={formData.nombre}
                onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
                placeholder={t('inventory.unitManagement.form.namePlaceholder')}
              />
            </div>

            {/* Abreviatura */}
            <div className="space-y-2">
              <Label>{t('inventory.unitManagement.form.abbreviation')}</Label>
              <Input
                value={formData.abreviatura}
                onChange={(e) => setFormData({ ...formData, abreviatura: e.target.value.toUpperCase() })}
                placeholder={t('inventory.unitManagement.form.abbreviationPlaceholder')}
                maxLength={5}
              />
              <p className="text-xs text-[#666666]">
                {t('inventory.unitManagement.form.abbreviationHelp')}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-[#666666]">
                {t('inventory.unitManagement.form.tareWeightHelp')}
              </p>
            </div>

            {/* Selector de Icono */}
            <div className="space-y-2">
              <Label>{t('inventory.unitManagement.form.icon')}</Label>
              ...
              <p className="text-xs text-[#666666]">
                {t('inventory.unitManagement.form.iconHelp', { count: iconosDisponibles.length })}
              </p>
            </div>

            {/* Vista Previa */}
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              <p className="text-xs text-[#666666] mb-2">{t('inventory.unitManagement.preview.label')}</p>
              <div className="flex items-center gap-3">
                <div className="text-3xl">{formData.icono}</div>
                <div>
                  <p className="font-medium" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                    {formData.nombre || t('inventory.unitManagement.preview.defaultName')}
                  </p>
                  <p className="text-sm text-[#666666]">
                    {formData.abreviatura || t('inventory.unitManagement.preview.defaultAbbreviation')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            >
              ...
              {t('inventory.unitManagement.actions.cancel')}
            </Button>
            <Button
            ...
            >
              ...
              {modoEdicion ? t('inventory.unitManagement.actions.save') : t('inventory.unitManagement.actions.create')}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
